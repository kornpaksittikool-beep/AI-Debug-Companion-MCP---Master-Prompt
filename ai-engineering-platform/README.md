...
Recommends a token-aware MCP evidence gathering flow for a repository task. It accepts an optional `questionType` (`project_summary`, `tech_stack_quick_view`, `debugging`, `code_review`, `planning`, or `general`) and returns a target token range, `gateMode`, excerpt `maxBytes`, excerpt call limits, start/evidence/escalation tools, and hard `doNotCallTools` guidance. For `project_summary`, startup mode skips `platform.tool_summary` when `repository.project_profile` is available, strict mode excludes broad search, symbol search, architecture docs, source tree summaries, app module excerpts, and full context reads from the preferred route, and the context policy tells clients to use the compact read-only Workflow Gate.
...
Returns the MCP workflow index for routing task types to tools, modules, and files. Each entry includes a target token range, `gateMode`, excerpt limits, hard do-not-call tools, and context policy so clients can keep summary, tech stack, debug, review, and planning workflows narrow. The `project_summary` entry uses `compact_read_only` gate mode with `No file changes` impact and `Not required: read-only` approval wording.
