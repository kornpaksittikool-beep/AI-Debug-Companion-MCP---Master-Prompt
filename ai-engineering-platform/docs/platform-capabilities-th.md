...
เอกสารนี้สรุปแบบภาษาคนว่า MCP ตัวนี้ทำอะไรได้บ้าง ณ Phase 31 โดยเน้นความสามารถที่ใช้ช่วยงานพัฒนา software จริง ไม่ใช่รายละเอียดเชิง implementation
...
- แยก profile ตามประเภทคำถาม พร้อม target โดยประมาณ:
  - summary: ประมาณ 1k-2k tokens
    - ใช้ excerpt ไม่เกิน 2 ครั้ง และ `maxBytes` ประมาณ 500-700
...
- workflow index แต่ละ entry มี target token range และ context policy เพื่อบังคับให้เริ่มจาก evidence ที่เล็กพอ
...
ค่าที่ควรเห็นจาก Phase 31:

- `platform.health` เป็น `ok`
- phase เป็น `phase-31-summary-project-profile-mode`
- จำนวน tools ทั้งหมดเป็น `84`
- ใช้ `platform.tool_summary` ได้
- `token_budget.recommend_strategy` คืน `questionProfile` สำหรับประเภทคำถามที่ระบุหรือ infer ได้
...
- summary/purpose prompt ควรเรียก `repository.search_files` ด้วย `mode=summary` และ `maxMatches<=8`
