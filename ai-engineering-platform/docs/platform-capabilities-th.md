...
เอกสารนี้สรุปแบบภาษาคนว่า MCP ตัวนี้ทำอะไรได้บ้าง ณ Phase 40 โดยเน้นความสามารถที่ใช้ช่วยงานพัฒนา software จริง ไม่ใช่รายละเอียดเชิง implementation
...
- workflow index แต่ละ entry มี target token range, `gateMode`, `defaultReportMode`, `debugReportTriggers`, context policy และ workflow acceptance criteria เพื่อบังคับให้เริ่มจาก evidence ที่เล็กพอและตรวจได้ว่าจบ workflow จริง
...
ค่าที่ควรเห็นจาก Phase 40:

- `platform.health` เป็น `ok`
- phase เป็น `phase-40-real-integration-workflows`
...
- summary/debug/review/planning workflow มี `workflowAcceptanceCriteria` สำหรับเงื่อนไขเริ่มงาน evidence boundary รูปแบบรายงาน และ verification
