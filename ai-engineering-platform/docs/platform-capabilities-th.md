...
เอกสารนี้สรุปแบบภาษาคนว่า MCP ตัวนี้ทำอะไรได้บ้าง ณ Phase 36 โดยเน้นความสามารถที่ใช้ช่วยงานพัฒนา software จริง ไม่ใช่รายละเอียดเชิง implementation
...
- workflow index แต่ละ entry มี target token range, `gateMode` และ context policy เพื่อบังคับให้เริ่มจาก evidence ที่เล็กพอ
...
ค่าที่ควรเห็นจาก Phase 36:

- `platform.health` เป็น `ok`
- phase เป็น `phase-36-adaptive-gate-compactness`
- จำนวน tools ทั้งหมดเป็น `84`
- ใช้ `platform.tool_summary` ได้
- `token_budget.recommend_strategy` คืน `questionProfile` พร้อม `gateMode` สำหรับประเภทคำถามที่ระบุหรือ infer ได้
- summary/purpose prompt ควรใช้ `gateMode=compact_read_only` และ gate สั้นประมาณ 4-5 บรรทัด
