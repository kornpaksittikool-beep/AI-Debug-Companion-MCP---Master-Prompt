...
| M1: Core MCP Framework | Phase 1 | NestJS application scaffold with strict TypeScript baseline | Phase 0 approval | Critical | Medium | Planned |
| M1: Core MCP Framework | Phase 1 | MCP transport boundary and core server abstraction | NestJS scaffold | Critical | High | Planned |
| M1: Core MCP Framework | Phase 1 | Tool registry with plugin registration contract | Core server abstraction | Critical | High | Planned |
| M1: Core MCP Framework | Phase 1 | Structured logging, request correlation, and execution timing | Core server abstraction | High | Medium | Planned |
| M1: Core MCP Framework | Phase 1 | Standard error envelope with reason and suggestion | Core server abstraction | High | Medium | Planned |
| M1: Core MCP Framework | Phase 1 | Basic health and platform metadata MCP tools | Tool registry | High | Low | Planned |
| M1: Core MCP Framework | Phase 1 | Unit and integration test baseline with coverage gate | Scaffold | Critical | Medium | Planned |
| M2: Investigation Engine | Phase 2 | Investigation session model with evidence, hypotheses, visited resources, and conclusion | Phase 1 | Critical | High | Planned |
| M2: Investigation Engine | Phase 2 | Problem classifier for errors, logs, stack traces, screenshots, issues, and feature requests | Investigation session model | High | Medium | Planned |
| M2: Investigation Engine | Phase 2 | Evidence registry with traceable source references | Investigation session model | Critical | High | Planned |
| M3: Repository Intelligence | Phase 3 | Repository scanner with ignore rules and path validation | Phase 1 | Critical | High | Planned |
| M3: Repository Intelligence | Phase 3 | Symbol and file search backed by an incremental index | Repository scanner | Critical | High | Planned |
| M3: Repository Intelligence | Phase 3 | AST parser plugin interface for TypeScript and JavaScript | Repository scanner | High | High | Planned |
| M3: Repository Intelligence | Phase 3 | Smart context reader for project, module, file, and function context | Symbol index | Critical | High | Planned |
| M4: Database Intelligence | Phase 4 | Database connection abstraction with explicit permissions | Phase 1 | High | High | Planned |
| M4: Database Intelligence | Phase 4 | Schema and relation discovery tools | Database abstraction | High | Medium | Planned |
| M4: Database Intelligence | Phase 4 | Query preview with read-only enforcement | Database abstraction | High | Medium | Planned |
